...
class Gujarati {
  ...
  static const String languageName = 'ગુજરાતી';
  ...
  List<String> gujtel = KeyBoardChars().Gujtel, guj = KeyBoardChars().guj;
  ...
  String getMappedText(String text) {
    ...
    for (int i = 0; i < text.length; i++) {
      int index = gujtel.indexOf(text[i]);
      if (index != -1) {
        ans += guj[index];
      } else {
    ...
    debugPrint('getMappedText() called for gujarati $ans');
    return ans;
  }
  ...
  void addTopChars(String char) {
    ...
    debugPrint('addTopChars() called for gujarati $char');
  }
  ...
  void removeTopChars() {
    ...
    debugPrint('removeTopChars() called for gujarati');
  }
