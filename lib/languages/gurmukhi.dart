...
class Gurmukhi {
  ...
  static const String languageName = 'ਗੁਰਮੁਖੀ';
  ...
  List<String> gurtel = KeyBoardChars().Gurtel, gur = KeyBoardChars().gur;
  ...
  String getMappedText(String text) {
    ...
    for (int i = 0; i < text.length; i++) {
      int index = gurtel.indexOf(text[i]);
      if (index != -1) {
        ans += gur[index];
      } else {
    ...
    debugPrint('getMappedText() called for gurmukhi $ans');
    return ans;
  }
  ...
  void addTopChars(String char) {
    ...
    debugPrint('addTopChars() called for gurmukhi $char');
  }
  ...
  void removeTopChars() {
    ...
    debugPrint('removeTopChars() called for gurmukhi');
  }
